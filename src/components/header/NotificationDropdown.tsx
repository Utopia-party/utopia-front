...
import { formatRelativeTime, getNotificationBadge, getNotificationTargetPath } from './headerUtils';
...
export function NotificationDropdown({
  ...
  const markOneMut = useMutation({
    ...
    onSuccess: (_, id) => {
      queryClient.setQueryData<NotificationItem[]>(notificationKeys.me, (prev = []) => {
        const next = markNotificationReadInList(prev, id);
        queryClient.setQueryData(notificationKeys.unreadCount, countUnreadNotifications(next));
        return next;
      });
    },
  });
      ...
      {isOpen && (
        <div className="absolute right-0 top-12 z-50 w-96 rounded-2xl border border-gray-200 bg-white p-3 shadow-xl">
          <div className="mb-3 flex items-center justify-between gap-3">
            <div>
              <h3 className="text-sm font-bold text-gray-800">{nickname ?? '사용자'}님 알림</h3>
              {unreadCount > 0 && <p className="mt-0.5 text-xs font-medium text-red-500">읽지 않음 {unreadCount}개</p>}
            </div>
            <button
              type="button"
          ...
          <div className="max-h-96 space-y-2 overflow-y-auto">
            {isLoading ? (
              <div className="rounded-xl bg-gray-50 px-4 py-6 text-center text-sm text-gray-500">알림을 불러오는 중...</div>
            ) : isError ? (
              <div className="rounded-xl bg-red-50 px-4 py-6 text-center text-sm text-red-500">알림을 불러오지 못했습니다.</div>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          {item.title && <p className="truncate text-sm font-semibold text-gray-800">{item.title}</p>}
                        </div>
                        <p className="mt-1 break-words whitespace-pre-line text-sm text-gray-700">{item.message ?? '알림 내용이 없습니다.'}</p>
                      </div>
                      {!item.is_read && <span className="mt-1 h-2 w-2 shrink-0 rounded-full bg-red-500" />}
                    </div>
              ...
              <div className="rounded-xl bg-gray-50 px-4 py-6 text-center text-sm text-gray-500">새 알림이 없습니다.</div>
            )}
          </div>
        </div>
      )}
