...
type PartyWithDetails = Party & { description?: string; host_trust_score?: number };

type MatchedParty = NonNullable<ComponentProps<typeof MatchingSuccessModal>['matchedParty']> & {
  id?: number | string;
};
...
export default function Home() {
  ...
  const [refreshKeys, setRefreshKeys] = useState<Record<string, number>>(() => {
    try { const raw = sessionStorage.getItem('party_refresh_keys'); return raw ? JSON.parse(raw) : {}; }
    catch { return {}; }
  });
  ...
  const { data: trendingKeywords = [], isLoading: isTrendingLoading } = useQuery({
  ...
  const handleSearchAction = (keyword: string) => {
    setSearch(keyword);
    if (keyword.trim()) {
      recordSearchKeyword(keyword.trim()).catch((err) => console.warn('검색어 기록 실패:', err));
    }
  };
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setCooldown((c) => {
        if (c <= 1) { clearInterval(timer); localStorage.removeItem(STORAGE_KEY); return 0; }
        return c - 1;
      });
    }, 1000);
  ...
  const handleRefresh = () => {
    ...
    setRefreshKeys((prev) => {
      ...
      try { sessionStorage.setItem('party_refresh_keys', JSON.stringify(next)); } catch { /* ignore */ }
      return next;
    });
  ...
  const { data: partyData, isLoading } = useQuery({
    ...
    queryFn: () => fetchParties({ category_name: category ?? undefined, search, size: 6, random: !search, refreshKey: currentRefreshKey }),
    staleTime: Infinity,
  });

  const parties = useMemo<PartyWithDetails[]>(() => {
    if (partyData && Array.isArray(partyData.parties)) return partyData.parties as PartyWithDetails[];
    return [];
  }, [partyData]);
  ...
  return (
    <div className="flex w-full min-w-0 flex-1 flex-col bg-slate-50">
      <section className="relative overflow-hidden bg-[radial-gradient(circle_at_top_left,rgba(255,255,255,0.18),transparent_28%),linear-gradient(135deg,#4f46e5_0%,#6366f1_42%,#0ea5e9_100%)] px-4 sm:px-6 py-8 sm:py-10 text-center">
        ...
        <div className="relative mx-auto max-w-3xl">
          ...
          <div className="relative mt-6">
            ...
            <KeywordChips keywords={trendingKeywords} isLoading={isTrendingLoading} onPick={handleSearchAction} />
          </div>
        </div>
      </section>
            ...
            <section className="min-w-0 flex-1">
              ...
              {isLoading ? (
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
                  {[...Array(6)].map((_, i) => (
                    <div key={i} className="h-72 animate-pulse rounded-2xl sm:rounded-3xl border border-slate-200 bg-white" />
                  ))}
                </div>
              ) : parties.length === 0 ? (
                <div className="flex flex-col items-center justify-center rounded-2xl sm:rounded-3xl border border-dashed border-slate-300 bg-white px-4 py-16 sm:px-6 sm:py-20 text-center shadow-sm">
                  <div className="mb-4 flex h-14 w-14 sm:h-16 sm:w-16 items-center justify-center rounded-full bg-slate-100 text-2xl sm:text-3xl">🔎</div>
                  <h3 className="text-base sm:text-lg font-extrabold text-slate-900">조건에 맞는 파티가 아직 없어요</h3>
                  <p className="mt-2 max-w-md text-sm leading-6 text-slate-500">검색어를 바꿔보거나, 직접 새 파티를 만들어 멤버를 모집해보세요.</p>
                  <div className="mt-6 flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
                    <button onClick={() => navigate('/handcaptcha')} className="w-full sm:w-auto rounded-xl sm:rounded-2xl bg-slate-900 px-5 py-3 text-sm font-bold text-white transition hover:bg-slate-800">파티 생성하기</button>
                    <button onClick={handleQuickMatchOpen} className="w-full sm:w-auto rounded-xl sm:rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-bold text-slate-700 transition hover:bg-slate-50">빠른 매칭 열기</button>
                  </div>
                </div>
              ) : (
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
                  {parties.map((party) => (
                    <PartyCard key={party.id} party={party} onDetail={setDetailTarget} onApply={setApplyTarget} />
                  ))}
                </div>
              )}
            </section>
      ...
      {detailTarget && (
        <PartyDetailModal party={detailTarget} onClose={() => setDetailTarget(null)}
          onApply={(p) => { setDetailTarget(null); setApplyTarget(p); }} />
      )}
      {applyTarget && <ApplyModal party={applyTarget} onClose={() => setApplyTarget(null)} />}

      <QuickMatchForm open={showQuickMatch} onClose={() => setShowQuickMatch(false)} onSubmit={handleQuickMatchSubmit} isSubmitting={isMatching} />
      <MatchingLoadingModal open={isMatching} message={currentStepTitle} />
      <MatchingErrorModal open={!!matchError} message={matchError ?? ''} errorCode={matchErrorCode ?? undefined} onClose={clearError} />
      <MatchingSuccessModal
        open={!!matchResult && !isMatching && !matchError}
        matchedParty={matchedParty}
        paymentPreview={matchPaymentPreview}
        onClose={clearResult}
        onGoParty={() => { if (matchedParty?.id) goToParty(matchedParty.id); }}
      />
    </div>
  );
}
