...
function normalizeErrorCode(error: unknown): string {
  ...
  if (raw === 'ALREADY_IN_ACTIVE_PARTY' || raw.includes('이미 참여 중인 활성 파티') || raw.includes('이미 가입') || raw.includes('활성 파티가 있습니다'))
    return 'ALREADY_IN_ACTIVE_PARTY';
  if (raw === 'ALREADY_REQUESTED' || raw.includes('이미 빠른매칭')) return 'ALREADY_REQUESTED';
...
export function useQuickMatchFlow() {
  ...
  const [matchPaymentPreview, setMatchPaymentPreview] = useState<PaymentPreviewResponse | null>(null);
  ...
  const currentStepTitle = (() => {
    switch (matchStep) {
      case 'requesting': return '빠른 매칭 요청을 확인하고 있어요';
      case 'finding': return '조건에 맞는 파티를 찾고 있어요';
      case 'selecting': return '가장 잘 맞는 파티를 고르고 있어요';
      case 'joining': return '선택된 파티에 자동으로 참여하는 중이에요';
      default: return '조건에 맞는 파티를 찾고 있어요';
    }
  })();
  ...
  const handleSubmit = async (payload: {
    ...
    try {
      ...
      const joinResponse = (await joinMut.mutateAsync(request_id)) as unknown as JoinResult;
      const matchedPartyId = joinResponse?.party_id ?? joinResponse?.id;

      if (!matchedPartyId) throw new Error('매칭된 파티 정보를 찾을 수 없습니다.');
      ...
      } catch { setMatchPaymentPreview(null); }

      let detailedParty: PartyDetailResponse | null = null;
      try { detailedParty = (await fetchParty(String(matchedPartyId))) as PartyDetailResponse; }
      catch { /* ignore */ }

      setMatchResult({
        ...joinResponse,
        ...(detailedParty ?? {}),
        party_id: (() => { const n = Number(joinResponse?.party_id ?? detailedParty?.id); return Number.isNaN(n) ? undefined : n; })(),
        party_title: joinResponse?.party_title ?? detailedParty?.title ?? '매칭된 파티',
        title: detailedParty?.title ?? joinResponse?.party_title ?? '매칭된 파티',
        service_name: detailedParty?.service_name ?? joinResponse?.service_name ?? '',
        category_name: detailedParty?.category_name ?? joinResponse?.category_name ?? '기타',
        member_count: detailedParty?.member_count ?? joinResponse?.member_count ?? 0,
        max_members: detailedParty?.max_members ?? joinResponse?.max_members ?? null,
        monthly_price: detailedParty?.monthly_price ?? joinResponse?.monthly_price ?? null,
        original_price: detailedParty?.original_price ?? joinResponse?.original_price ?? null,
        service_total_price: detailedParty?.service_total_price ?? joinResponse?.service_total_price ?? null,
        host_nickname: detailedParty?.host_nickname ?? joinResponse?.host_nickname ?? '익명',
        host_trust_score: detailedParty?.host_trust_score ?? joinResponse?.host_trust_score ?? null,
        description: detailedParty?.description ?? joinResponse?.description ?? '',
  ...
  const clearResult = () => { setMatchResult(null); setMatchPaymentPreview(null); };
  const clearError = () => { setMatchError(null); setMatchErrorCode(null); };
