...
const pipeline = [
  {
    step: '1단계',
    name: '블랙리스트 필터',
    desc: '키워드 직접 매칭. 화이트리스트 포함 시 즉시 통과.',
  ...
  },
  {
    step: '2단계',
    name: 'KR-ELECTRA (ML)',
    desc: 'none 확률 ≥ 0.95 → 통과 / score ≥ 0.97 → 즉시 차단. 평균 추론 24.4ms.',
  ...
  },
  {
    step: '3단계',
    name: 'Ollama LLM',
    desc: '2단계 경계 케이스 재판단. 자기비하·감탄 표현 등 문맥 파악.',
      ...
      <div className="mb-14">
        ...
        <div className="stagger-grid grid grid-cols-1 md:grid-cols-3 gap-4">
          {datasets.map((d) => (
            <div
              key={d.name}
              className="hover-lift rounded-2xl border border-gray-100 bg-gray-50 p-6 will-change-transform"
            >
          ...
          <table className="w-full min-w-[640px] text-sm text-left">
            ...
            <tbody className="divide-y divide-gray-50 bg-white">
              {experiments.map((exp) => (
                <tr
                  key={exp.round}
                  className={exp.isBest ? 'bg-indigo-50/60' : 'hover:bg-gray-50/50'}
                >
                  ...
                  <td className={`px-4 py-3 font-bold tabular-nums ${exp.isBest ? 'text-indigo-600' : 'text-gray-800'}`}>
                    {exp.f1Macro.toFixed(4)}
                  </td>
        ...
        <div className="stagger-grid grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
          <div className="hover-lift rounded-2xl bg-indigo-50 border border-indigo-100 p-5 will-change-transform">
            ...
            <p className="text-sm text-gray-700 leading-relaxed">
              roberta-base → roberta-large 전환 시 F1-Macro{' '}
              <strong>+2.35%p</strong> 향상. KR-ELECTRA는 ELECTRA Discriminator 구조로 학습 속도 약{' '}
              <strong>3배 빠름</strong>.
            </p>
          </div>
          <div className="hover-lift rounded-2xl bg-purple-50 border border-purple-100 p-5 will-change-transform">
            ...
            <p className="text-sm text-gray-700 leading-relaxed">
              전 실험 Epoch 2에서 최고점 후 하락 → LR Overshooting 판단. 2e-5 → 1e-5 조정 후{' '}
              <strong>안정적 학습</strong> 확인.
            </p>
          </div>
          <div className="hover-lift rounded-2xl bg-yellow-50 border border-yellow-100 p-5 will-change-transform">
            ...
            <p className="text-sm text-gray-700 leading-relaxed">
              APEACH ↔ nayohan 간 <strong>85% 중복</strong> 발견 → APEACH 제거. K-MHaS는 datasets 라이브러리 지원 중단으로 제외.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col md:flex-row gap-3 items-stretch">
          {pipeline.map((p, i) => (
            <div key={p.step} className="flex flex-col md:flex-row items-center gap-3 flex-1">
              <div
                className={`hover-lift w-full rounded-2xl border p-6 ${p.borderColor} bg-white will-change-transform`}
              >
                <span className={`inline-flex px-3 py-1 rounded-full text-xs font-bold mb-3 ${p.color}`}>
                  {p.step}
                </span>
