...
const steps = [
  {
    step: '1',
    name: '요청 생성',
    desc: '사용자·서비스·이용 기간 저장 중복 요청 및 활성 파티 가입 여부 검증',
    table: 'quick_match_requests',
  },
  {
    step: '2',
    name: '하드필터',
    desc: '정원 초과·이미 가입·신뢰도 미달·기간 불일치·신고/정지/비활성 상태 정책 필터링',
    table: 'quick_match_candidates',
  },
  {
    step: '3',
    name: '후보 점수 계산',
    desc: 'rule_score와 probability_score를 계산한 뒤 final_score로 정렬',
    table: 'rule_score / final_score',
  },
  {
    step: '4',
    name: '최종 후보 선택',
    desc: 'final_score 1위 후보를 선택하고 결과 스냅샷 저장',
    table: 'quick_match_results',
  },
  {
    step: '5',
    name: '실제 가입',
    desc: '선택 파티에 PartyMember를 생성하고 빠른매칭 가입 이벤트 기록',
    table: 'party_members / training_events',
  },
  {
    step: '6',
    name: '운영 결과 수집',
    desc: '정산·탈퇴·강퇴·신고·제재·유지기간 데이터 누적',
    table: 'result_snapshot',
  },
  {
    step: '7',
    name: '라벨링 / 통계 집계',
    desc: 'pending 이벤트를 success/failed/excluded로 확정 후 통계 테이블 재집계',
    table: 'quick_match_training_stats',
  },
];
...
const scores = [
  ...
  {
    name: '통계기반 점수(probability_score)',
...
const labels = [
  {
    ...
    desc: '30일 유지, 정산 성공, 신고/제재/탈퇴/강퇴 없음',
    used: true,
  },
  {
    label: 'failed',
    ...
    pct: 28.3,
    color: 'bg-rose-400',
    desc: '3일 내 탈퇴/강퇴, 정산 실패/노쇼, 신고/제재 발생',
    used: true,
  },
  {
    label: 'pending',
...
const statAxes = [
  {
    key: 'global',
    example: 'all',
    desc: '전체 평균 성공률 표본 부족 bucket의 fallback 기준',
  },
  { key: 'service', example: 'service_id', desc: '서비스별 성공률 반영' },
  {
    key: 'trust_bucket',
    example: 'under_30 / 30_40 / 40_50 / 50_60 / over_60',
    desc: '신뢰도 구간별 성공률 반영',
  },
  {
    key: 'duration_match',
    example: 'exact / overlap / boundary / mismatch',
    desc: '선호 기간과 파티 기간 적합도별 성공률',
  },
  {
    key: 'capacity_bucket',
    example: 'high / medium / low / full',
    desc: '남은 자리 비율에 따른 성공률',
  },
];
    ...
    >
      ...
      <div className="section-header flex flex-col items-center text-center mb-16">
        ...
        <p className="text-gray-500 text-base md:text-lg max-w-2xl mx-auto leading-relaxed">
          ...
          운영 로그가 쌓이면 통계 기반 probability_score 비중을 점진적으로
          높이는 구조입니다.
        </p>
      </div>
              ...
              <div key={s.step} className="timeline-item relative">
                ...
                <div className="hover-lift rounded-2xl border border-gray-100 bg-white shadow-sm p-5 will-change-transform">
                  <div className="flex flex-col md:flex-row md:items-center gap-2 mb-1">
                    ...
                    <span className="md:ml-auto px-2 py-0.5 rounded-full bg-gray-100 text-gray-500 text-[11px] font-mono">
                      {s.table}
                    </span>
                  </div>
      ...
      <div className="mb-14">
        ...
        <div className="stagger-grid grid grid-cols-1 md:grid-cols-3 gap-4">
          {scores.map((s) => (
            <div
              key={s.name}
              className={`hover-lift rounded-2xl border p-6 will-change-transform ${s.color}`}
            >
              <div className="flex items-center justify-between mb-3">
                <p className="font-black text-gray-900 font-mono">{s.name}</p>
                <span
                  className={`px-2 py-0.5 rounded-full text-xs font-bold ${s.tagColor}`}
                >
                  {s.weight}
                </span>
              </div>
      ...
      <div className="mb-14">
        ...
        <p className="text-xs text-gray-400 mt-3 bg-amber-50 border border-amber-100 rounded-xl px-4 py-3">
          success + failed({totalLabeled.toLocaleString()}건)만 통계 기반 성공률
          계산에 사용. pending은 관찰 기간 종료 전 보류, excluded는 매칭 품질
          판단 제외.
        </p>
      </div>
            ...
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                {['집계 축', '예시 key', '활용 방식'].map((h) => (
                  <th key={h} className="px-4 py-3 font-bold text-gray-600">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-gray-50 bg-white">
              {statAxes.map((a) => (
                <tr key={a.key} className="hover:bg-gray-50/50">
                  <td className="px-4 py-3 font-bold text-green-700 font-mono">
                    {a.key}
                  </td>
      ...
      <div>
        ...
        <div className="stagger-grid grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <div
              key={p.phase}
              className={`hover-lift rounded-2xl border p-6 will-change-transform ${p.color}`}
            >
              <div className="flex items-center justify-between mb-3">
                <p className="text-xs font-bold text-gray-400">{p.phase}</p>
                <span
                  className={`px-2 py-0.5 rounded-full text-[11px] font-bold ${p.tagColor}`}
                >
                  {p.tag}
                </span>
              </div>
