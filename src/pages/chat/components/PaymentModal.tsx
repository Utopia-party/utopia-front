...
export function PaymentModal({
  ...
  const hasLeaderDiscount = isLeader && !!leaderDiscountRate && leaderDiscountRate > 0;
  ...
  const handleCardPayment = async () => {
    ...
    try {
      ...
      if (!response) { alert('결제가 취소되었습니다.'); return; }
      if (response?.code) { alert(`결제 실패: ${response.message ?? '알 수 없는 오류'}`); return; }
    ...
    <div className="bg-green-50 border border-green-200 rounded-xl p-3 flex flex-col gap-1.5">
      {hasLeaderDiscount && (
        <div className="flex justify-between text-sm">
          ...
          <span className="text-green-700 font-bold">-{Math.round((leaderDiscountRate ?? 0) * 100)}%</span>
        </div>
      )}
      {hasReferrerDiscount && (
        <div className="flex justify-between text-sm">
          <span className="text-green-700 font-medium">추천인 할인</span>
          <span className="text-green-700 font-bold">-{Math.round((referralDiscountRate ?? 0) * 100)}%</span>
        </div>
      )}
      <div className="flex justify-between text-xs text-green-600 border-t border-green-200 pt-1.5 mt-0.5">
        <span>총 할인</span>
        <span className="font-bold">-{Math.round(discountRate * 100)}% ({(base - payAmount).toLocaleString()}원 절약)</span>
      </div>
    </div>
  );
      ...
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
          ...
          <div className="p-8 flex flex-col items-center text-center gap-4">
            ...
            <button onClick={onClose} className="mt-2 w-full py-3 bg-slate-900 text-white rounded-2xl text-sm font-bold hover:bg-slate-800">
              확인
            </button>
          </div>
        </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="bg-slate-50 rounded-xl px-4 py-3 flex justify-between text-sm">
                ...
                <div className="flex items-center gap-2">
                  {hasDiscount && <span className="text-xs text-slate-400 line-through">{base.toLocaleString()}원</span>}
          ...
          {step === 'card' && (
            <div className="flex flex-col gap-4">
              <div className="bg-slate-50 rounded-xl p-4 flex flex-col gap-2">
                ...
                {hasLeaderDiscount && (
                  <div className="flex justify-between text-sm">
                    <span className="text-green-600 font-medium">방장 할인</span>
                    <span className="text-green-600 font-bold">-{Math.round((leaderDiscountRate ?? 0) * 100)}%</span>
                  </div>
                )}
                {hasReferrerDiscount && (
                  <div className="flex justify-between text-sm">
                    <span className="text-green-600 font-medium">추천인 할인</span>
                    <span className="text-green-600 font-bold">-{Math.round((referralDiscountRate ?? 0) * 100)}%</span>
                  </div>
                )}
