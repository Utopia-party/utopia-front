...
export function NicknameField({
  nickname, nicknameError, nicknameSuccess, isGenerating, onChange, onRandom,
}: NicknameFieldProps) {
  return (
    <div>
      ...
      <div className="relative">
        <input
          name="nickname" type="text" placeholder="닉네임 입력" value={nickname}
          className="w-full rounded-lg border border-gray-300 p-3 pr-28 focus:border-blue-500 focus:outline-none"
          onChange={onChange}
          required
        />
        <button
          type="button" onClick={onRandom} disabled={isGenerating}
      ...
      {!nicknameError && nicknameSuccess && <p className="mt-1 text-xs text-green-600">{nicknameSuccess}</p>}
    </div>
  );
}
