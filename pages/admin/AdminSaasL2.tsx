...
export default function AdminSaasL2() {
  const SERVICE_TYPE = 'captcha_l2' as const;
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">캡챠 SaaS (L2)</h1>
            <p className="mt-1 text-sm text-gray-500">손동작 + OCR 기반 멀티모달 실시간 검증 API 키 관리</p>
          </div>
          <button
      ...
      {modal.type === 'create' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <h3 className="mb-4 text-lg font-bold">캡챠 SaaS (L2) 키 발급</h3>
            <form onSubmit={handleCreate} className="space-y-3">
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button type="submit" className="flex-1 rounded-lg bg-violet-600 py-2 text-sm font-medium text-white hover:bg-violet-700">발급</button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            ...
            <form onSubmit={handleEdit} className="space-y-3">
              ...
              <div className="flex items-center gap-2">
                <input type="checkbox" id="is_active_l2" checked={editForm.is_active}
                  onChange={(e) => setEditForm((p) => ({ ...p, is_active: e.target.checked }))} />
                <label htmlFor="is_active_l2" className="text-sm text-gray-700">활성화</label>
              </div>
              <div className="flex gap-2 pt-2">
                <button type="button" onClick={() => setModal({ type: null })} className="flex-1 rounded-lg border py-2 text-sm hover:bg-gray-50">취소</button>
