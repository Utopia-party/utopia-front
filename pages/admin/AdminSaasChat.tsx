...
import {
  ...
  updateSaasV2PlanInquiry,
  type SaasKeyItem,
  type UsageLogItem,
  type SaasStats,
} from '../../apis/admin/adminSaasV2';

type ModalState = { type: 'create' | 'edit' | 'rotate' | 'logs' | 'inquiries' | null; keyId?: string };
...
export default function AdminSaasChat() {
  const SERVICE_TYPE = 'chat_filter' as const;

  const [keys, setKeys] = useState<SaasKeyItem[]>([]);
  const [stats, setStats] = useState<SaasStats | null>(null);
  const [logs, setLogs] = useState<UsageLogItem[]>([]);
  const [inquiries, setInquiries] = useState<{ id: string; user_id: string; desired_plan: string; message: string | null; status: string; created_at: string }[]>([]);
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [keysData, statsData] = await Promise.all([
        fetchSaasV2Keys({ service_type: SERVICE_TYPE, search: search || undefined, is_active: filterActive }),
        fetchSaasV2Stats(SERVICE_TYPE),
      ]);
  ...
  const openInquiries = useCallback(async () => {
    try {
      const data = await fetchSaasV2PlanInquiries({ service_type: SERVICE_TYPE, status: 'pending' });
      setInquiries(data.items);
      setModal({ type: 'inquiries' });
    } catch (err) { setError((err as Error).message); }
  }, []);

  const handleInquiryAction = useCallback(async (id: string, action: 'approved' | 'rejected') => {
    try {
      await updateSaasV2PlanInquiry(id, { status: action });
      const data = await fetchSaasV2PlanInquiries({ service_type: SERVICE_TYPE, status: 'pending' });
      setInquiries(data.items);
    } catch (err) { setError((err as Error).message); }
  }, []);
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">채팅 AI SaaS</h1>
            <p className="mt-1 text-sm text-gray-500">Ollama LLM 기반 욕설 / 비매너 실시간 탐지 API 키 관리</p>
          </div>
          <button
        ...
        <div className="flex flex-wrap gap-2">
          <div className="relative flex-1 min-w-48">
            ...
            <input
              className="w-full rounded-lg border py-2 pl-9 pr-3 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400"
      ...
      {modal.type === 'create' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <h3 className="mb-4 text-lg font-bold">채팅 AI SaaS 키 발급</h3>
            <form onSubmit={handleCreate} className="space-y-3">
              <div>
                <label className="mb-1 block text-xs text-gray-500">서비스명 *</label>
                <input className="w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" className="w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400"
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button type="submit" className="flex-1 rounded-lg bg-emerald-600 py-2 text-sm font-medium text-white hover:bg-emerald-700">발급</button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            ...
            <form onSubmit={handleEdit} className="space-y-3">
              ...
              <div className="flex items-center gap-2">
                <input type="checkbox" id="is_active_chat" checked={editForm.is_active}
                  onChange={(e) => setEditForm((p) => ({ ...p, is_active: e.target.checked }))} />
                <label htmlFor="is_active_chat" className="text-sm text-gray-700">활성화</label>
              </div>
              <div className="flex gap-2 pt-2">
                <button type="button" onClick={() => setModal({ type: null })} className="flex-1 rounded-lg border py-2 text-sm hover:bg-gray-50">취소</button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            ...
            <button onClick={() => { setModal({ type: null }); setRotatedSecret(null); }}
              className="mt-4 w-full rounded-lg bg-emerald-600 py-2 text-sm font-medium text-white hover:bg-emerald-700">확인</button>
          </div>
        </div>
      )}
      ...
      {modal.type === 'inquiries' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-xl">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-lg font-bold">플랜 업그레이드 문의 (대기)</h3>
              <button onClick={() => setModal({ type: null })}><X className="h-5 w-5 text-gray-400" /></button>
            </div>
            {inquiries.length === 0 ? (
              <p className="py-8 text-center text-sm text-gray-400">처리할 문의가 없습니다.</p>
            ) : (
              <div className="space-y-3 max-h-96 overflow-y-auto">
                {inquiries.map((inq) => (
                  <div key={inq.id} className="rounded-lg border p-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <span className="text-sm font-medium text-gray-900">희망 플랜: {inq.desired_plan}</span>
                        <p className="text-xs text-gray-400 mt-0.5">{inq.created_at?.slice(0, 10)}</p>
                        {inq.message && <p className="mt-1 text-sm text-gray-600">{inq.message}</p>}
                      </div>
                      <div className="flex gap-2">
                        <button onClick={() => handleInquiryAction(inq.id, 'approved')}
                          className="rounded-lg bg-green-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-green-700">승인</button>
                        <button onClick={() => handleInquiryAction(inq.id, 'rejected')}
                          className="rounded-lg bg-red-500 px-3 py-1.5 text-xs font-medium text-white hover:bg-red-600">거절</button>
                      </div>
                    </div>
                  </div>
                ))}
